...
export const ForwardContractsView: React.FC<ForwardContractsViewProps> = ({
  ...
  React.useEffect(() => {
    const fetchContracts = async () => {
      try {
        // Assume api is imported from '@/lib/axios'
        // But since this file doesn't have it imported, we use the global fetch or a prop
        // We will add import api from '@/lib/axios' at the top later
        const res = await fetch('http://127.0.0.1:8080/api/supplier/contracts/forward');
        const json = await res.json();
        if (json.success && json.data && json.data.content) {
          const mapped = json.data.content.map((c: any) => ({
            id: c.id?.toString() || `fc-${Date.now()}`,
            cropPlanId: 'crop-1', // Mock mapped
            cropName: c.product?.name || 'Sản phẩm',
            plotName: 'Lô A1',
            buyerName: 'Đối tác',
            buyerCompany: 'Công ty Đối Tác',
            buyerPhone: '0909090909',
            requestedQuantityTons: c.targetVolumeKg ? c.targetVolumeKg / 1000 : 5,
            proposedPriceVnd: c.fixedPrice || 85000,
            depositPercent: c.depositPercentage || 20,
            expectedDeliveryDate: c.expectedDeliveryDate || '2026-10-15',
            notes: c.description || '',
            status: c.status === 'OPEN' ? 'pending' : (c.status === 'REGISTERED' ? 'accepted' : 'completed'),
            createdAt: c.createdAt || 'Vừa xong',
