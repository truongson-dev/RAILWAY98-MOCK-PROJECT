...
import { X, ShieldCheck, Lock, CheckCircle2, DollarSign, Clock, ArrowRight } from 'lucide-react';
...
interface EscrowModalProps {
  isOpen: boolean;
  onClose: () => void;
  selectedProduct?: AgProduct | null;
}
...
export const EscrowModal: React.FC<EscrowModalProps> = ({
  ...
  const currentProduct = selectedProduct
    ? {
        name: `${selectedProduct.availableQuantityTons} Tấn ${selectedProduct.name}`,
        value: selectedProduct.pricePerKg * selectedProduct.availableQuantityTons * 1000,
      }
    : {
        name: contractState.productName,
        value: contractState.totalValueVND,
      };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-[#f7fbf0] w-full max-w-3xl rounded-3xl shadow-2xl border border-[#e0e4d9] max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg sm:text-xl font-bold font-sans">
                Hợp Đồng Thanh Toán Tạm Khóa Escrow
              </h2>
              <p className="text-xs text-emerald-100">
                Mã HĐ: <strong className="font-mono text-white">{contractState.id}</strong>
              </p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          {showSuccessToast && (
            <div className="p-4 bg-emerald-100 border border-emerald-300 text-emerald-900 rounded-2xl flex items-center gap-3 text-sm font-semibold animate-bounce">
              <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
              <span>Thành công! Tiền ký quỹ Escrow đã được giải ngân 100% cho Nông trại sau khi kiểm định đạt chuẩn!</span>
            </div>
          )}
          ...
          <div className="bg-white border border-[#e0e4d9] rounded-2xl p-5 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#f1f5ea] pb-4">
              ...
              <div className="text-left sm:text-right">
                <span className="text-xs text-[#707a6c] font-medium block">Tổng giá trị giao dịch:</span>
                <span className="text-xl font-bold text-[#176a22]">
                  {currentProduct.value.toLocaleString('vi-VN')} VNĐ
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3 bg-[#f1f5ea] rounded-xl border border-[#e0e4d9]">
                <span className="text-[#707a6c] block">Bên mua (Buyer):</span>
                <strong className="text-[#181d16] text-sm block mt-0.5">{contractState.buyerName}</strong>
              </div>
              <div className="p-3 bg-[#f1f5ea] rounded-xl border border-[#e0e4d9]">
                <span className="text-[#707a6c] block">Bên bán (Seller):</span>
          ...
          <div className="bg-[#176a22] text-white p-6 rounded-2xl space-y-4 shadow-md">
          ...
          {/* Milestones List */}
          <div className="bg-white border border-[#e0e4d9] rounded-2xl p-5 space-y-4">
            <h4 className="text-sm font-bold text-[#181d16] font-sans flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3">
                    <CheckCircle2
                      className={`w-5 h-5 shrink-0 ${
                        m.completed ? 'text-[#176a22]' : 'text-gray-300'
                      }`}
                    />
          ...
          <div className="pt-2 flex items-center justify-between">
            <p className="text-xs text-[#707a6c] max-w-sm">
              Tiền được tạm giữ an toàn tại Ngân hàng đối tác Vietcombank / BIDV cho đến khi biên bản nghiệm thu chất lượng hoàn tất.
            </p>
